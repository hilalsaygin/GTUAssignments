...
{
    // if there is a defined interrupthandler object for the given interrupt id 
    if(handlers[interrupt] != 0)
    {
        ///call HandleInterrupt for defined interrupts

        esp = handlers[interrupt]->HandleInterrupt(esp);
    }
    //timer interrupt degilse enter 
    else if(interrupt != hardwareInterruptOffset)
    {   
    }
    //if timer interrupt occurs robin scheduler
